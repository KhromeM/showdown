...
export default class BattleHandler {
	...
	handleRequest(request) {
		...
		if (request.side) {
			this.bot.gameState.myTeam.pokemon = request.side.pokemon.map(
				(pokemon) => ({
					ident: pokemon.ident,
			...
			this.bot.gameState.myTeam.active = request.side.pokemon.findIndex(
				(p) => p.active
			);

			const activePokemon = request.side.pokemon.find((p) => p.active);
			if (activePokemon) {
				this.ourPokemonName = activePokemon.ident.split(": ")[1];
	...
	handleSwitch(parts) {
		...
		// 	this.bot.decisionMaker.battleLog("We switched to: " + ourPokemonName);
		// }

		if (pokemon !== this.ourPokemonName) {
			// this.bot.decisionMaker.battleLog(
			// 	"Opponent switched to:" + JSON.stringify(pokemon)
			// );
			...
			if (
				!this.bot.gameState.opponent.knownTeam.some(
					(p) => p.pokemon === pokemon
				)
			) {
	...
	handleMove(parts) {
		...
		if (pokemon !== this.ourPokemonName) {
			this.bot.decisionMaker.battleLog("Opponent move:", move, "from", pokemon);
